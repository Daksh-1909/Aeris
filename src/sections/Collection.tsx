...
export function Collection({ onOpen }: { onOpen: (photo: Photograph, photos: Photograph[]) => void }) {
  const [category, setCategory] = useState<GalleryCategory>('All');
  const [photos, setPhotos] = useState<Photograph[]>([]);
  useEffect(() => { void getPhotographs(category).then(setPhotos); }, [category]);
  return <section className="collection section-wrap" id="collection">
    ...
    <div className="collection-toolbar"><div className="filters" aria-label="Filter photographs">{categories.map((item) => <button key={item} className={category === item ? 'is-active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div><span>{String(photos.length).padStart(2, '0')} photographs</span></div>
